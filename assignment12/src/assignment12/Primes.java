...
{	
    public static void main(String args[])
    {
        ...
        /*creating three lists of candidates,composites and primes number*/
        ...
        while(candidates.isEmpty()!=true) 
        {
        	int p=candidates.remove(1);
        	primes.add(p);
        	getComposites(candidates,composites,p);
        }
    ...
    public static void getComposites(ListInterface<Integer> candidates, ListInterface<Integer> composites, Integer prime)
    {
    	for(int i=1;i<=candidates.getLength();i++)
        {
    		if((candidates.getEntry(i))%prime==0)
    			composites.add(candidates.remove(i)); //adding composite number to list by checking with prime no.
        }
    }
